...
static bool is_empty_slot(List table_entry) {
    return table_entry && list_empty(table_entry);
}
...
static bool find_key(Pointer *array, size_t size, Pointer key, size_t key_hash, ListNode *entry_node, List *table_list) {
    assert(array != NULL);
...
size_t calculate_ht_size(size_t entries) {
    // we want a load factor of 0.1 tops -> 10/100 -> 1/10 so:
    return entries*10;
}

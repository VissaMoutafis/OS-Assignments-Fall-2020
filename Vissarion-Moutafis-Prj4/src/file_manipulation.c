...
// function that copies a file ( with name src_path) to the file with name trg_path. 
// If the latter doesn't exist create it.
int clean_copy_file(char *src_path, char *trg_path, int BUFFSIZE, char *out_root_path) {
    int in, out;
    // first we must check if the src_path is a hard link and if it is then we must 
    ...
    if (is_sym(src_path)) {
        if (manage_links)
            return create_symlink(src_path, trg_path, out_root_path);
        else 
            return FILE_CP_SUCC;
    }
    ...
    if (manage_links && number_of_links(src_path) > 1 && create_link(src_path, trg_path, out_root_path, inodes_map) == FILE_CP_SUCC)
        return FILE_CP_SUCC;

    // open input file
    if ((in = open(src_path, O_RDONLY)) == -1) {
    ...
    if ((out = open(trg_path, O_WRONLY | O_CREAT | O_TRUNC)) == -1) {
        fprintf(stderr, "%s: Cannot open output file.\n", src_path);
        return FILE_CP_FAIL;
    }

    // copy the mode of in at the out's inode
    if (chmod(trg_path, path_get_mode(src_path)) < 0) {
        perror("chmod");
        return FILE_CP_FAIL;
    }
    ...
    if (verbose)
        print_copy_element(trg_path, out_root_path);
...
int copy_dir(char *src_path, char *trg_path, int BUFFSIZE, char *out_root_path) {
    DIR *in_dp, *out_dp;
    struct dirent *dirent_in;
    
    // first open the directory 
    if ((in_dp = opendir(src_path)) == NULL) {
        fprintf(stderr, "Trouble opening directory '%s'.\n", src_path);
        return DIR_CP_FAIL;
    }
    // Now try to open the target dir
    if ((out_dp = opendir(trg_path)) == NULL) {
        // at this case the dir does not exist so we must create it
        out_dp = create_dir(trg_path);
        if (!out_dp) {
    ...
    while ((dirent_in = readdir(in_dp)) != NULL) {
        if (strcmp(".", dirent_in->d_name) == 0 || strcmp("..", dirent_in->d_name) == 0)
            continue;
        char *element_path = calloc(strlen(src_path) + 1 + strlen(dirent_in->d_name) + 1, sizeof(char));
        char *new_trg_path = calloc(strlen(trg_path) + 1 + strlen(dirent_in->d_name) + 1, sizeof(char));

        // now we have to create the new src_path 
        strcpy(element_path, src_path);
        strcat(element_path, "/");
        strcat(element_path, dirent_in->d_name);

        // now we have to create the new trg_path
        strcpy(new_trg_path, trg_path);
        strcat(new_trg_path, "/");
        strcat(new_trg_path, dirent_in->d_name);

        // // copy every element of the directory
        if (copy_element(element_path, new_trg_path, out_root_path) == FAIL) {
            closedir(in_dp);
            closedir(out_dp);
            return DIR_CP_FAIL;
        }
    ...
    if (check_for_deleted && check_deleted(src_path, trg_path, out_root_path) == FAIL) {
        closedir(in_dp);
        closedir(out_dp);
        return DIR_CP_FAIL;
    }
...
// The trg_path is the new pathname that you give to the deep copy you make.
...
int copy_element(char *src_path, char *trg_path, char *trg_root_path) {
    ...
    items_detected += 1;
    if (element_exists(trg_path) && !is_same_type(src_path, trg_path))
        delete_element(trg_path, trg_root_path);

    if (is_dir(src_path)) {
        return copy_dir(src_path, trg_path, CP_BUFFSIZE, trg_root_path);
    } else {
        // in case the src and dest files are not different then there is no
        // reason to copy again
        if (files_diff(src_path, trg_path) == 0)
            return FILE_CP_SUCC;

        

        // printf("copy %s\n", src_path);
        return clean_copy_file(src_path, trg_path, CP_BUFFSIZE, trg_root_path);
    }
...
void print_statistics(void) {
    ...
    printf("Deleted %d items from target hierarchy.\n", items_deleted);
}
